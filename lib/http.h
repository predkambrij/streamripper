...
error_code http_construct_sc_request(const char *url, const char* proxyurl, char *buffer, char *useragent, char *referer);
...
error_code http_sc_connect (RIP_MANAGER_INFO* rmi,
			    ...
			    const char *proxyurl, 
			    SR_HTTP_HEADER *info, char *useragent, char *referer, 
			    char *if_name);
